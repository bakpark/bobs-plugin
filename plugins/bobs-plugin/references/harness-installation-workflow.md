...
§5.2 의 *입력 trigger 3종* 과 동일 — 명시 호출 / 자동 chain / hook 트리거 (간접 — hook 자체는 사용자 알림만, runner 호출은 사용자 또는 main session 결정).

### Chain 절차 (main session 책임)

1. runner 호출 (진입 조건 중 하나) → `mode: cycled | no-op | needs_input | blocked` + `routing_decision` 반환 (산출 4 섹션 — §5.2 Output Contract 참조)
...
3. design skill 산출 spec (또는 자체 작성 결과) 반영 후 다시 runner 호출 (자동 chain, `round` 증가)
4. 종료 조건 충족까지 반복
...
- 사용자 명시 종료 ("stop", "충분", "지금까지", "그만")
- 같은 target + 같은 gap fingerprint 가 2회 연속 (재진입 무한 루프 신호 — fingerprint = `(target, case_id, result, summary_hash)` tuple, 정의는 `evaluation-loop-runner/references/runtime-protocol.md` §2.1 참조)
- 누적 라운드 5회 초과 (`NEEDS_REVIEW` — 사용자에게 핸드오프)

종료 후 main session 은 사이클 결과를 사용자에게 한 줄 요약 + 마지막 task log entry path 안내. 상세 절차는 runner 의 `references/runtime-protocol.md` §6 참조.

### Cycle 카운터 책임

- 라운드 카운트 (`round` 필드) 는 *main session* 이 유지 — runner 는 stateless (이전 호출 모름, 매 호출 `round` 입력으로 받음)
- 동일 fingerprint 2회 연속 검출은 main session 이 *직전 라운드의 `(target, gap_fingerprint)` tuple 1개* 유지하면 됨 (chain 이력 전체 보관 불요 — fingerprint 정의는 runner `runtime-protocol.md` §2.1)
- 종료 조건 위반 시 main session 이 chain 중단 + 사용자에게 사유 보고
- self-application (runner 가 자기 자신 작성 entry 평가) 검출도 main session 책임 — 2회 이상 self-application 시 `NEEDS_REVIEW` 핸드오프 (runner `runtime-protocol.md` §5)
...
| 6 | runner 사이클 종료 조건 무시 | 같은 target+fingerprint 2회 연속 또는 5회 초과 라운드를 main session 이 chain 계속 | §6 종료 조건 4종 강제 — main session 의 cycle 카운터 책임 (§6 *Cycle 카운터 책임* + runner `runtime-protocol.md` §2.1 fingerprint 정의) |
...
| 8 | docs/agent body 재생산 | workflow doc 또는 runner 본문이 `docs/agent/evaluation-loop.md` Routing Decision 표 본문 / `golden-set.md` case / `task-log-template.md` schema 를 재생산 (drift 원인) | workflow doc / runner 는 *인용 path* 만, 본문 재생산 금지 — Step 4 의 ownership 분리 패턴 (design = 진실 source, runtime = read then apply) |
...
| should-trigger | "사이클 진입" / "/evaluation-loop-runner" 명시 호출 | §5.2 trigger 1 → `evaluation-loop-runner` 진입 (Phase 1-3 한 묶음) |
