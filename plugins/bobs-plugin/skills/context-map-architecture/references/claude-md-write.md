...
> **책임 경계 주의**: 아래 6 criteria 표는 원본 (Anthropic `claude-md-improver`, Apache-2.0) 의 일반 CLAUDE.md 평가 rubric 을 그대로 보존. 본 프로젝트의 harness 책임 모델 (§4.1) 에서는 `Commands/workflows` / `Architecture clarity` / `Testing approaches` 가 **`AGENTS.md` 소유**다. CLAUDE.md 평가 시 이들 criterion 에 점수가 높게 잡힌다면 → *책임 누수 신호*. AGENTS.md 로 이동 권고 + CLAUDE.md 자체는 `Conciseness` / `Currency` / `Actionability` / `Non-obvious patterns` (Claude 한정 운용 지침 관점) 위주로 재평가한다.
...
> **책임 경계 주의**: 아래 섹션 목록은 원본 (Apache-2.0) 의 일반 CLAUDE.md 권장 골격 그대로. 본 프로젝트 모델에서 `Commands` / `Architecture` / `Key Files` / `Testing` / `Environment` 는 **`AGENTS.md` 소유** (작성 절차는 `agents-md-write.md`). CLAUDE.md 에 직접 넣지 말고 `AGENTS.md` 의 해당 섹션을 한 줄로 *링크* 한다 — 예: "작업 명령 / 검증 / 금지 행동은 `AGENTS.md` 참조". CLAUDE.md 본문은 `Code Style` (Claude 가 따를 톤·판단 원칙), `Gotchas` (Claude 한정 함정), `Workflow` (Claude 사용 시 "어디서 시작 → 무엇 호출") 만 남기는 것이 권장.

- **Commands** — build / test / dev / lint 표 *(AGENTS.md 소유 — CLAUDE.md 에는 두지 말 것)*
- **Architecture** — 디렉토리 트리 + 각 dir 한 줄 purpose *(AGENTS.md / docs/architecture.md 소유)*
- **Key Files** — entry point, config 파일 + purpose *(AGENTS.md 소유)*
- **Code Style** — 프로젝트 컨벤션 (Claude 가 적용할 톤·판단 원칙)
- **Environment** — 필수 환경 변수 + setup step *(AGENTS.md 소유 — CLAUDE.md 에는 두지 말 것)*
- **Testing** — test 명령 + 작동하는 testing 패턴 *(AGENTS.md 소유)*
- **Gotchas** — non-obvious quirk / 흔한 실수 (Claude 가 자주 놓치는 함정 중심)
- **Workflow** — 작업 패턴 ("X 할 때는 Y" — Claude 가 처음 진입할 때 라우팅 힌트)
