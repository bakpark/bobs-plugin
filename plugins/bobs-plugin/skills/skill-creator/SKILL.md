...
- runtime 사이클 실행 (task log entry write + golden-set 비교 + Routing Decision) → `evaluation-loop-runner`. 본 스킬은 *skill 작성* 만, runtime 실행 아님.
...
- **args 입력 contract** (§0) — main session 의 design skill spec dispatch 진입점. `name` / `scope` 사전 채움 허용, intent 본질 질문 (#1-#5) 은 args 로 우회 불가 — backward-compatible (사용자 직접 호출 동작 보존).
