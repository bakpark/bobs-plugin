...
관련 자산: `writing-skills`(discipline), `writing-agents`(서브에이전트), `agent-skill-designer`(타입·책임 결정), `agent-skill-auditor`(정적 rule 감사).
...
| 신호 | 위반 | 전환 대상 |
|---|---|---|
| 별도 context / tool 권한 / 별도 model 이 필요한 specialist 역할 | §3.5 — 스킬은 메인 context 에 로드됨 | `writing-agents` |
