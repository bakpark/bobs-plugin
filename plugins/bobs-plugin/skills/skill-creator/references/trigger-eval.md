...
description 은 스킬 발견의 일차 신호다. 호출자가 catalog 에서 스킬을 보고 *지금 자동 capability 를 로드해야 할지* 결정한다. trigger 정확도 측정은 description 이 의도한 케이스에서 트리거되고 다른 케이스에서 트리거되지 않는지 확인한다.

## CSO 핵심 원칙

### 1. Description = When to Use, NOT What the Skill Does

description 은 *자동 활성화 조건* 만 담는다. 본문 절차를 요약하면 호출자가 본문을 안 읽고 description 만 따라 단축 실행한다 (Description-as-runbook 안티패턴).
...
# ✅ GOOD: 자동 capability 조건만
description: Use when transforming Open-Meteo API responses into canonical weather JSON for downstream commands or agents
```
...
특히 should-not-trigger 에 다음을 포함한다:
- 사용자가 직접 시작하는 workflow 나 plan gate 가 필요한 요청. 이 경우 command 가 맞다.
- 프로젝트 문서 링크를 모아 context 로 주입하는 요청. 이 경우 command 또는 context map 이 맞다.
- 별도 context/tool/model 격리가 필요한 specialist 판단. 이 경우 agent 가 맞다.
...
따라서 trigger eval 쿼리는 *Claude 가 외부 인프라·API·provider·도메인 capability 참조로 이득을 볼 만큼* 복잡해야 한다. 단순 1-step 쿼리나 사용자 명시 workflow 요청은 description 품질 무관하게 스킬 트리거 평가에 부적합하다.
