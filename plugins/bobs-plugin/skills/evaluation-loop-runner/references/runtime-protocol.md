...
| 3 | 같은 target + 같은 gap fingerprint 2회 연속 | 직전 호출의 `(next_action.target, gap_fingerprint)` 와 현재 호출의 동일 (§2.1 정의) | "사이클 종료 — `<skill>` + 동일 gap 2회 연속 (재진입 무한 루프 신호)" |
| 4 | 누적 5회 초과 | `round > 5` | "NEEDS_REVIEW — 누적 5 라운드 초과, 사용자 핸드오프" |

조건 #3 검출을 위해 main session 은 *직전 라운드의 (target, gap_fingerprint) tuple 1개* 만 유지하면 됨 (chain 이력 전체 X). 라운드 카운트는 정수 1개.

### 2.1 gap_fingerprint 정의

같은 target 의 정상 재진입 (예: 첫 round 는 golden-set 추가, 두 번째 round 는 task-log-template 정밀화 — 같은 `evaluation-loop-design` 으로 들어가지만 다른 gap) 을 false positive 로 stop 하지 않기 위해 fingerprint 까지 비교.

**fingerprint = 4 필드 tuple** (runner 의 Output Contract `routing_decision` + `gap_analysis.*` 에서 추출):

| 필드 | source | 정규화 |
|---|---|---|
| `target` | `next_action.target` | trim, lowercase |
| `case_id` | `gap_analysis.case_id` | trim |
| `result` | `gap_analysis.result` (PASS / FAIL / no-op / blocked / needs_input) | enum 그대로 |
| `summary_hash` | `gap_analysis.summary` 의 정규화 hash | 공백 단일화 + lowercase 후 hash (sha1 또는 짧은 fingerprint 함수) |

**비교 규칙**: 4 필드 모두 일치하면 `same_fingerprint == true` → 조건 #3 trigger. 셋 중 하나라도 다르면 정상 재진입 (다른 cycle), chain 계속.

### 2.2 비교 예시

**false positive 회피 (chain 계속)** — 같은 target, 다른 case_id:

```yaml
# Round N
target: evaluation-loop-design
case_id: golden-set-missing
result: FAIL
summary_hash: a1b2c3

# Round N+1 (같은 target 인데 다른 gap)
target: evaluation-loop-design
case_id: task-log-schema-drift
result: FAIL
summary_hash: d4e5f6
# → fingerprint 불일치 (case_id 다름) → chain 계속
```

**true positive (chain stop)** — 같은 target + 같은 case_id + 같은 result + 같은 summary:

```yaml
# Round N
target: resource-design
case_id: resource-type-ambiguous
result: needs_input
summary_hash: a1b2c3

# Round N+1 (같은 gap 반복 — 진전 없음)
target: resource-design
case_id: resource-type-ambiguous
result: needs_input
summary_hash: a1b2c3
# → fingerprint 일치 → 조건 #3 trigger → chain stop
```

**`case_id: unknown` 처리**: free-form task_type (case 정의 없음) 의 fingerprint 는 `summary_hash` 비교가 주 기준. unknown case 가 2회 연속 같은 summary 면 stop.
...
| 종료 조건 4종 enforce (no-op / 사용자 명시 / 같은 target+fingerprint 2회 연속 / 5회 초과) | main session |
