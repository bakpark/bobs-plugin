...
| `event` | `PostToolUse` / `Stop` / `UserPromptSubmit` / `PreToolUse` / `PostCompact` 등 | 본 §0 의 #2 *언제 트리거* 질문 사전 채움. 채워지면 skip |
| `matcher` | event 별 — `PostToolUse` 의 tool 이름 / `Stop` 의 reason 등. 비워두면 모든 매칭 | §0 #3 matcher 질문 + §2 settings.json registration 작성 input |
| `scope` | `user` / `project` / `plugin` (plugin 은 `plugins/<plugin>/hooks/<name>/hooks.json` 디렉토리 단위 등록 — §1 Scope 표 행 3 참조) | 본 §0 의 6번 질문 사전 채움 |

args 키가 누락된 채 호출되면 *해당 키의 §0 질문만* 사용자에게 묻는다. intent 본질 질문 (#1 책임 / #4 exit policy / #5 부수 효과) 은 args 로 우회 불가 — hook 의 보안·성능·결정론 결정의 핵심.
...
- **args 입력 contract** (§0) — main session 의 design skill spec dispatch 진입점. `name` / `event` / `matcher` / `scope` 사전 채움 허용, intent 본질 질문 (#1 책임 / #4 exit policy / #5 부수 효과) 은 args 로 우회 불가 — backward-compatible (사용자 직접 호출 동작 보존).
- 외부 모델 호출·네트워크 IO 없음.
