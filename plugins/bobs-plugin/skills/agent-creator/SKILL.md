...
관련 자산: `writing-agents`(discipline + pressure scenario 절차), `writing-skills`(스킬 작성 — 비교 reference), `skill-creator`(스킬 sibling), `agent-skill-designer`(타입·책임 결정), `agent-skill-auditor`(정적 rule 감사).
...
- 자원 타입(skill / agent / hook) 결정·책임 분리·migration plan → `agent-skill-designer`.
...
| 신호 | 위반 | 전환 대상 |
|---|---|---|
| 반복 가능한 판단 절차 / reference bundle 이지 specialist role 이 아님 | §3.5 — 에이전트는 별도 context + tool/model 격리 specialist | `skill-creator` |
| 매 이벤트마다 결정론 보장 필요 | §3.1 — 에이전트는 명시 dispatch, hook 은 결정론 | `harness-resource-design` (hook 트랙) |
| 프로젝트 고유 규칙 | §3.6 — reusable 자산이 아닌 local convention | `CLAUDE.md` |
| 한 파일이 분석·수정·commit·deploy 다 수행 (generalist) | §3.5 + §3.3 — single responsibility 위반 | 책임을 둘 이상으로 분리하거나 `agent-skill-designer` 위임 |
| 다른 에이전트를 디스패치하는 orchestrator | §3.5 — orchestration 은 caller 책임 | 호출자(skill / main session) 로 이동 |
...
2. frontmatter 초안 — *agent 지원 필드만* 사용 (`name`, `description`, `tools`, `model`, 선택적 `color`). `disable-model-invocation` / `user-invocable` / `allowed-tools` 는 skill 전용이므로 agent 에 두지 않는다.
3. tool/model 결정 근거 — read-only vs +Write vs +Bash, sonnet vs opus vs haiku 의 한 줄 설명
...
- **Tool/Model boundary IS discipline** — 스킬은 본문 prose 가 곧 discipline 이지만, 에이전트는 *frontmatter 의 `tools:` 와 `model:` 자체가 discipline 의 절반* 이다. advisory 역할이 `tools: *` 면 본문에 "수정하지 않는다" 라고 써도 압력 상황에서 모델은 도구를 사용한다. tool allowlist 는 safety boundary 이지 documentation 이 아니다.
- **Iron Law (RED before GREEN)** — baseline 실패 없이 작성된 에이전트는 *어떤 drift 를 막는지* 알 수 없다. 작성 전 generic subagent 로 baseline prompt 를 dispatch 해 무엇이 실패하는지 (role drift / tool drift / output drift / dispatch drift) 관찰한다. 자세한 절차는 `${CLAUDE_PLUGIN_ROOT}/skills/agent-creator/references/red-green-refactor.md`.
- **함정 10종 (AGENT-GUIDE §13)** — Generalist agent / All-tools reviewer / No scope / No output contract / Low-confidence spam / Description bloat / Agent as runbook / Hidden mutation / Project convention leak / Unintentional duplicate. 작성 후 §13 표로 자기 검열. writing-agents 의 Always-opus / Orchestrator agent / Persona drift mid-body / Auto-merge generator / No negative case / Multi-language persona 도 함께 검토한다.
...
- **tools**: 역할에 맞는 explicit allowlist. read/review/audit 은 `Read, Grep, Glob, LS` 가 기본. generation 은 `+ Write, Edit`. infra/migration 은 `+ Bash`. `tools: *` 는 catch-all 빌트인이 아닌 한 안티패턴 (writing-agents §All-tools agent).
...
- 3 라운드까지: 평소 흐름.
- 3 라운드 초과: §0 의 *책임* 정의로 복귀 — 책임 모호가 진짜 원인. 또는 자원 타입이 잘못 잡혔는지 (agent 가 아닌 skill/hook/CLAUDE.md) 재검토.
...
2. **자원 타입 오류** → 전환. agent 가 아니라 skill / hook / CLAUDE.md 가 적합한 경우. *에이전트 특화 신호*: tool 격리도 model 격리도 별도 context 도 필요 없는 단순 절차 → skill.
3. **GUIDE_GAP** → 가이드가 좋은 자산을 적절히 평가하지 못함. 사용자에게 보고하고 자산은 통과시킨다. 가이드 보완은 별도 작업.
4. **NEEDS_REVIEW** → 근거가 부족하거나 추정이 많은 경우. 사용자 입력 후 재개.

보조 도구로 RED-GREEN-REFACTOR — pressure scenario (실패 유도 입력) 로 자산이 실패하는 경우를 찾는다. 자세한 절차는 `${CLAUDE_PLUGIN_ROOT}/skills/agent-creator/references/red-green-refactor.md` (writing-agents 의 관련 섹션을 self-contained 형태로 가져옴).

## Description optimization (선택)

GAP 분석은 *영향 기준* 평가다. description 의 *트리거 정확도* 는 별도 측정이 필요한 경우가 있다 — 인접 에이전트와 키워드 충돌 의심 시 (예: "review", "analyze", "audit" 가 여러 에이전트에 분포). 절차는 `${CLAUDE_PLUGIN_ROOT}/skills/agent-creator/references/trigger-eval.md` (writing-agents 의 CSO 섹션을 self-contained 형태로 가져옴). 비용·시간 소요 — 사용자 동의 후.
...
- 작성한 에이전트의 *behavior verification* (RED-GREEN-REFACTOR pressure dispatch) 은 본 스킬의 의무가 아니라 *선택* — 호출자가 명시적으로 요청하거나 §When the loop stalls 의 보조 도구로 사용. writing-agents §Iron Law 가 강하게 권장하나 본 스킬은 *작성·GAP 통과* 까지 책임.
