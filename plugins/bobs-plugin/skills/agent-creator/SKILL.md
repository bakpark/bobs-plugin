...
name: agent-creator
description: |-
  Use when creating, scaffolding, editing, or verifying a Claude Code subagent (`.md` under `agents/`). Triggers on "create an agent", "scaffold a subagent", "에이전트 만들어줘", "서브에이전트 추가", "agent 작성·개선·검증", "/agent-name 만들어줘", "draft an agent for X" — including when the user has not yet chosen a name, persona, tools, or model. Do NOT use for writing skills (`skill-creator`), agent-vs-skill / merge / migration-order decisions (`resource-design`), static rule audit of an existing agent (`agent-skill-auditor`), or PR/code edits. Accepts `name` / `scope` / `subagent_type` args from design skill spec `Execution Plan` (optional pre-fill — intent questions #1-#5 always asked).
---
...
- runtime 사이클 실행 (task log entry write + golden-set 비교 + Routing Decision) → `evaluation-loop-runner`. 본 스킬은 *agent 작성* 만, runtime 실행 아님.
...
- **args 입력 contract** (§0) — main session 의 design skill spec dispatch 진입점. `name` / `scope` / `subagent_type` 사전 채움 허용, intent 본질 질문 (#1-#5) 은 args 로 우회 불가 — backward-compatible (사용자 직접 호출 동작 보존).
