...
관련 자산: `skill-creator`(스킬 sibling — 같은 6 phase 구조), `hook-creator`(훅 sibling), `writing-skills`(vendored 스킬 작성 discipline — 비교 reference), `agent-skill-designer`(타입·책임 결정), `agent-skill-auditor`(정적 rule 감사).
...
- **AGENT-GUIDE §13 안티패턴 16종** — Generalist agent / All-tools reviewer / No scope / No output contract / Low-confidence spam / Description bloat / Agent as runbook / Hidden mutation / Project convention leak / Unintentional duplicate / Always-opus / Orchestrator agent / Persona drift mid-body / Auto-merge generator / No negative case / Multi-language persona. 작성 후 §13 표로 자기 검열.
