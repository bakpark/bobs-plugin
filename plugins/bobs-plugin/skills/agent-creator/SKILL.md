...
- 자원 타입(command / skill / agent / hook / runtime setting) 결정·책임 분리·migration plan → `agent-skill-designer`.
...
| 신호 | 위반 | 전환 대상 |
|---|---|---|
| 사용자 명시 workflow / 사용자 질문 / plan gate / 문서 링크 context 주입 / 얕은 orchestration | §3.11 — workflow entrypoint 는 command | `agent-skill-designer` (command 트랙) |
