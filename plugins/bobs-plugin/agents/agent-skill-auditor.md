...
- `scope=user` — `~/.claude/agents/*.md` + `~/.claude/skills/*/SKILL.md` + `~/.claude/settings.json` 전체.
...
- **P0** (작동 불능·과권한·미통제 부수효과·보안 차단):
  frontmatter 파싱 불능 / 부수효과 스킬에 `disable-model-invocation: true` 누락 (S-M5) / 에이전트 `tools: *`·omit + Write/Bash 사용 (A-N1+) / 훅 secret 송신 (H-X3) / PostToolUse non-zero exit 로 작업 차단 (§8) / 광범위 path 매칭으로 작업 전체 차단 (H-M4).
- **P1** (MUST NOT·명확성/토큰 영향):
  Description-as-runbook (S-X1) / Description-bloat (`<example>` ≥3 또는 ≥2000자) / 본문 1000 words 초과 / `@path` 자동로드 (S-X2) / 페르소나+Output Guidance 동시 누락 (A-M4+A-M6) / 다중 자원 Always-opus (A-X2).
- **P2** (SHOULD 일반·권고 초과):
