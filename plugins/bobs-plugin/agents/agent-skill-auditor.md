...
description: |-
  사용자가 command `.md`, SKILL.md, agent `.md`, settings.json hooks/runtime 설정의 정적 감사·점검을 요청하거나, 호출자가 draft 자산 검토를 위임할 때 호출. Read-only로 CONSTITUTION/GUIDE 기준 P0/P1/P2 findings, confidence, metrics, rule evidence를 보고한다. Do NOT use for 설계·작성·수정, creation-time GAP loop(`creator-gap-eval`), evaluation design/runtime, trigger eval, 외부 모델/PR/code review, Dead asset 분석.
